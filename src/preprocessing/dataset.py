...
class DatasetHelper:
    @staticmethod
    def concat_and_split(csv_paths: List[str], train_csv_path: str, test_csv_path: str, shuffle: bool = False):
    ...
    @staticmethod
    def train_test_split(train_size: float, df: Optional[pd.DataFrame] = None, csv_path: Optional[str] = None,
    ...
    @staticmethod
    def data_loaders_from_datasets(train_dataset: Dataset, val_dataset: Dataset, batch_size: int, pin_memory=False) \
            -> Tuple[DataLoader, DataLoader]:
        train_data_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False, num_workers=0,
...
class HuggingFaceDataset:
    ...
    @staticmethod
    def hf_dataset(df: pd.DataFrame, remove_columns_list: List[str], identifier: str, batch_size: int = 8) -> Dataset:
        """
    ...
    def __process(auto_tokenizer, batch: Dict):
        ...
        inputs = tokenizer(batch['Normal'], padding='max_length', truncation=True,
                           max_length=HuggingFaceDataset.encoder_max_length)
        outputs = tokenizer(batch['Simple'], padding='max_length', truncation=True,
...
class DatasetManager:
    ...
    def create_asset_csv(source_folder_path: str, output_file_path: str):
        ...
        with open(f'{source_folder_path}/train.orig.txt', 'r') as train_orig_file:
            for line_index, orig_line in enumerate(train_orig_file):
                for i in range(10):
                    with open(f'{source_folder_path}/train.simp.{i}.txt') as train_simp_file:
                        simple_lines = train_simp_file.readlines()
                        df = df.append({'Normal': orig_line, 'Simple': simple_lines[line_index]}, ignore_index=True)

        with open(f'{source_folder_path}/valid.orig.txt', 'r') as valid_orig_file:
            for line_index, orig_line in enumerate(valid_orig_file):
                for i in range(10):
                    with open(f'{source_folder_path}/valid.simp.{i}.txt') as valid_simp_file:
    ...
    def create_one_stop_csv_from_txt(txt_path: str, output_file_path: str):
        ...
        df = pd.DataFrame(list(zip(rows[0::2], rows[1::2])), columns=['Normal', 'Simple'])
        df.to_csv(output_file_path)
    ...
    def create_simpa_csv(source_folder_path: str, output_file_path: str):
        ...
        for simpa_type in tqdm(['ls', 'ss']):
            with open(f'{source_folder_path}/{simpa_type}_normal.txt', 'r') as normal_file:
                with open(f'{source_folder_path}/{simpa_type}_simplified.txt', 'r') as simple_file:
                    simple_lines = simple_file.readlines()
                    for line_index, line in enumerate(normal_file):
                        df = df.append({'Normal': line, 'Simple': simple_lines[line_index]}, ignore_index=True)
    ...
    def create_ss_csv(input_file_path: str, output_file_path: str):
        ...
        with open(input_file_path, 'r') as file:
            for line in tqdm(file):
                ...
                df = df.append({'Normal': split[0], 'Simple': split[1]}, ignore_index=True)

        df.to_csv(output_file_path)

    @staticmethod
    def create_wiki_large_csv(normal_file_path: str, simple_file_path: str, output_file_path: str,
                              shuffle: bool = False):
        """
        ...
        with open(simple_file_path, 'r') as simple_file:
            with open(normal_file_path, 'r') as normal_file:
                ...
                for index, simple_line in tqdm(enumerate(simple_file)):
                    df = df.append({'Normal': normal_lines[index], 'Simple': simple_line}, ignore_index=True)
    ...
    @staticmethod
    def create_wiki_split_csv(tsv_file_path: str, output_csv_path: str, shuffle: bool = True, clean_up: bool = False,
...
class TextDataset:
    ...
    def __init__(self, path: str, train_file: str, test_file: str, batch_size: int, device: torch.device,
                 max_length: int):
        ...
        normal = data.Field(sequential=True, use_vocab=False, tokenize=TextDataset.__tokenize, pad_token=0,
                            fix_length=max_length)
        simple = data.Field(sequential=True, use_vocab=False, tokenize=TextDataset.__tokenize, pad_token=0,
                            is_target=True, fix_length=max_length)
    ...
    def iterators(self):
        train_iterator, test_iterator = data.BucketIterator.splits((self.train_data, self.test_data),
