...
class DataPreparer:
    @staticmethod
    def remove_low_similarity_records(
        threshold: float, df: pd.DataFrame
    ) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    old = os.path.abspath(
        __file__
        + "/../../../data/_collections/proc_w_s_2_columns_shuffle_with_meta_data.csv"
    )
    path = os.path.abspath(
        __file__
        + "/../../../data/_collections/proc_w_s_2_columns_shuffle_with_meta_data_without_low_similarity.csv"
    )
    # df = pd.read_csv(path)
