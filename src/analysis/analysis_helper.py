...
class AnalysisHelper:
    def __init__(self, doc2vec_model_path: str, dataframe: Optional[pd.DataFrame] = None,
        ...
        if csv_load_path is not None:
            ...
            print("`csv_load_path` and `dataframe` not set! DataFrame must be provided with `add_df`.")
    ...
    def stop_words_count(self):
        ...
        self.df['Normal_stop_word_count'] = self.df['Normal'].apply(lambda x: AnalysisHelper.__stop_word_count(x))
        self.df['Simple_stop_word_count'] = self.df['Simple'].apply(lambda x: AnalysisHelper.__stop_word_count(x))
    ...
    def cosine_distance(self):
        ...
        self.df['Cosine_distance'] = self.df.progress_apply(lambda row: self.__cosine_distance(row), axis=1)

    def cosine_similarity(self):
        print("'Cosine_similarity':")
        self.df['Cosine_similarity'] = self.df.progress_apply(lambda row: self.__cosine_similarity(row), axis=1)

    def sentence_cosine_similarity(self):
        print("'Sentence_similarity':")
        self.df['Sentence_similarity'] = self.df.progress_apply(lambda row: self.__sentence_similarity(row), axis=1)
